...
class SLOT(attrs.Attribute):
    ...
    def define(
        ...
        assert not any([issubclass(c, ComponentIter) for c in component_or_systems]), f'`ComponentIter` slot should be defined with `define_iterator` '
        assert all(
            [
    ...
    @classmethod
    def define_iterator(
        cls, *component_or_systems: ITERATOR, none_ok=False, default_ok=True, wide=True
    ):
