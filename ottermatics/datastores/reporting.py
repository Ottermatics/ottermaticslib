...
class TableBase(MappedDictMixin, ReportBase):
    ...
    store_type = {String: str, Integer: int, Numeric: float, Boolean: bool}
                ...
                if key in cols:  # we will map the data to the column
                    ...
                    if in_stypes:
                        ...
                        if not ctype in self.check_types:
                            valids = list(
                                filter(
                                    lambda ct: issubclass(ctype, ct),
                                    list(self.check_types.keys()),
                                )
                            )
                            ...
                            if valids:
                                if len(valids) > 1:
                                    self.warning(
                                        "more than one type matched for!!! {ctype}->{valids} "
                                    )
                                use_ctype = valids[0]
                    ...
                    if in_stypes and in_type:
                        ...
                        log.debug(
                            f"{key}={val} {vtype}=>{use_ctype}? col-valid:{in_stypes} valid-type:{in_type} col:{col}"
                        )
    ...
    def all_fields(cls):
        ...
        all_attr_fields = set(
            [
                attr.lower()
                for attr in cls._mapped_component.cls_all_attrs_fields().keys()
            ]
        )
...
class ResultsRegistry(Configuration, metaclass=SingletonMeta):
    ...
    def validator_to_column(self, attr_obj):
        if type(attr_obj.validator.type) is tuple and any(
    ...
    def dict_attr(
        self, comp_cls, target_table_id, results_table=None, is_analysis=False
    ):
        ...
        if issubclass(comp_cls, Analysis):
            table_abrv = self.analysis_attr_abrv
        name = comp_cls.__name__.lower()

        default_attr = {
            "__tablename__": f"{table_abrv}{name}",
        ...
        if (
            ...
            default_attr["analysis"] = relationship(
                root_obj.__name__, backref=backref(backref_name, lazy="noload")
            )
            ...
            backref_name = f"db_comp_attr_{name}"
            default_attr["result_id"] = Column(
                Integer, ForeignKey(target_table_id), primary_key=True
            )
    ...
    def default_attr(self, comp_cls, results_table=None):
        ...
        if issubclass(comp_cls, Analysis):
            table_abrv = self.analysis_table_abrv
        ...
        if results_table is not None:
            ...
            default_attr["run_id"] = Column(
                String(36)
            )  # corresponds to uuid set in analysis
    ...
    def component_attr(self, comp_cls, results_table=None):
        ...
        component_attr = {
            key.lower(): self.validator_to_column(field)
            for key, field in comp_cls.cls_all_attrs_fields().items()
            if self.filter_by_validators(field)
        }

        component_attr.update(
            {
                k.lower(): Column(k.lower(), Numeric(), nullable=True)
    ...
    def db_component_dict(
        ...
        if is_analysis:
            results_table = comp_attr["__tablename__"]  # pass reference for id!
        ...
        if is_analysis:  # We create the analysis first
            ...
            comp_db = self.check_or_create_db_type(
                comp_attr["__tablename__"], comp_db_tup
            )

            dict_attr = self.dict_attr(
                comp_cls,
                f"{comp_attr['__tablename__']}.id",
                results_table=results_table,
                is_analysis=is_analysis,
            )
            dict_name = f"{comp_cls_name}_Dict"
            dict_db_tup = (dict_name, (AttrBase,), dict_attr)
            dict_db = self.check_or_create_db_type(
                dict_attr["__tablename__"], dict_db_tup
            )
        ...
        return {
            comp_attr["__tablename__"]: comp_db,
    ...
    def check_or_create_db_type(self, tablename, type_tuple):
        ...
        if self.db.engine.has_table(tablename):
            ...
            self.debug(
                f"mapped existing table to type {tablename} -> {cls_db.__name__}"
            )
            ...
            self.debug(
                f"created table type {cls_db.__name__} -> {cls_db.__tablename__}"
            )
    ...
    def ensure_component_table(self, component_cls, results_table=None):
        ...
        is_analysis = False
        if issubclass(component_cls, Analysis):
            is_analysis = True

        self.info(f"ensuring component {component_cls} analysis?:{is_analysis}")
        db_component_type_dict = self.db_component_dict(
            component_cls, results_table=results_table, is_analysis=is_analysis
        )
        ...
        for comp_tbl, compdb in db_component_type_dict.items():
            if not compdb.__name__.endswith("Dict"):
                self.map_component(compdb, component_cls)
    ...
    def ensure_analysis(self, analysis):
        ...
        out = [
            dbckey
            for dbckey in db_component_type_dict.keys()
            if dbckey.startswith(self.analysis_table_abrv)
        ]
        if out:
            ...
            if analysis is analysis_cls:
                ...
                self.ensure_component_tables_in_analysis(
                    analysis, results_table=analysis_table
                )
        else:
    ...
    def _upload_analysis(self, analysis):
        ...
        class AvoidDuplicateAbortUpload(Exception):
            pass
...
class MappedItem(ReportBase):
    ...
    def __init__(self, tablerep, mapped_class):
        ...
        self.mro_inheritance = ".".join(
            [cls.__name__ for cls in distinct[0 : comp_inx + 1]]
        )
