...
class ClientSecrets(Secrets,ClientInfoMixin):
    ...
    def local_credential_files(self):
        ...
        paths = [ os.path.join(root,acc) for acc in itertools.accumulate(rel_paths, func=os.path.join, initial=root ) ]
        ...
        for path in paths:
            ...
            mtch = [ fil for fil in fils if os.path.isdir(fil) and any(list(map(lambda sec: os.path.basename(fil) == sec,self.creds_folders))) ]
            
            for mtc in mtch:
                stagepaths = [ os.path.join(mtc,fil) for fil in os.listdir( mtc) if os.path.isdir(os.path.join(mtc,fil)) ]

                for stagepath in stagepaths:
                    
                    stage = os.path.basename(stagepath)

                    stage_contents = [ os.path.join(fp,fil) for fp, _, fils in os.walk(stagepath) for fil in fils if any(list(map(fil.endswith,self.secret_scripts))) ]
                ...
                contents = [ os.path.join(mtc,fil) for fil in os.listdir( mtc) if any(list(map(fil.endswith,self.secret_scripts))) ]
    ...
    def local_credential_packages(self):
        ...
        for stage,cred_file_dict in stage_file_list.items():
            ...
            raw_dict = { decredential_file(key): creds for key, creds in cred_file_dict.items() if creds}
        
            #TODO: Ensure all credentials are from the same client (or exclusively in our company)
            # assert all(list(map(  )))

            paths_contains_creds=lambda path:[cred for key,creds in raw_dict.items() for cred in creds if key in path]
    ...
    def sync(self):
        ...
        if self.SLACK_WEBHOOK_NOTIFICATION is None and 'SLACK_WEBHOOK_NOTIFICATION' in os.environ:
