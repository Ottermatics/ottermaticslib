...
class TestCompLog(LoggingMixin):
    pass
...
class SpaceMixin(SolveableInterface):
    ...
    lenA = Solver.con_ineq(
        "max_length", "combine_length", combos="ineq_length", active=False
    )
...
class SliderCrank(System, CostModel):
    ...
    ro_slv.add_var_constraint(
        lambda sys, prob: sys.max_size / 2, "max", combos="max_size"
    )
    ...
    size = Solver.con_ineq(
        lambda sys, prob: sys.max_size, "overall_length", combos="max_size"
    )
    ...
    def end_force_diff(self) -> float:
        x = self.motion_curve
        return (
            (x.max() - self.X_spring_center) - (x.min() - self.X_spring_center)
        ) ** 2
    ...
    def Freact_y(self) -> float:
        return (
            np.sin(self.gamma) * np.cos(self.gamma) * self.Fslide_tot
            + self.F_external_y
        )
