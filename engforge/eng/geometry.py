...
class ParametricSpline:
    ...
    @functools.cached_property
    def a3(self):
        return (self.P2ds - self.P1ds - 2 * self.a2) / 3.0
...
def get_mesh_size(inst):
    ...
    return max(ms**2.0, dAmin)  # length to area conversion
...
class ShapelySection(Profile2D):
        ...
        if self.prediction:
            ...
            if self._symmetric and self._use_symmetric:
                self._prediction_models = {
                    "fails": {
                        "mod": svm.SVC(C=2000, gamma=0.1, probability=True),
                        "N": 0,
                    },
                    "fail_frac": {"mod": svm.SVR(C=5, gamma=0.5), "N": 0},
                }
            else:
                self._prediction_models = {
                    "fails": {
                        "mod": svm.SVC(C=5000, gamma=5, probability=True),
                        "N": 0,
                    },
                    "fail_frac": {"mod": svm.SVR(C=10, gamma=0.5), "N": 0},
                }
    ...
    def training_callback(self, models):
        ...
        if os.path.exists(self.meta_path):
            ...
            if test_score < meta["test_score"]:
                self.info(
                    f'new model score {test_score} is worse than {meta["test_score"]}, not saving'
                )
                return
    ...
    def init_with_material(self, material=None):
        ...
        if isinstance(self.shape, Geometry):
            ...
            if self._geo.material and self.material:
                self.warning(
                    f"overriding material {self._geo.material} with {self.material}"
                )
    ...
    def estimate_stress(
        ...
        if do_calc or force_calc:
            ...
            if self._do_print:
                self.info(
                    f'{"calc" if do_calc or oob else "est"} stress {abs(err):5.3f}>{mrg*calc_margin:5.3f}| {calc_margin} | oob {oob}'
                )

            if do_calc:
                return self.calculate_stress(
                    n=n, vx=vx, vy=vy, mxx=mxx, myy=myy, mzz=mzz, value=value
                )
    ...
    def solve_fail(self, fail_parm, base_kw, guess=None, tol=1e-4, mult=1):
        ...
        ans = skopt.root_scalar(
            self.fail_learning,
            x0=guess * 0.1,
            x1=guess,
            xtol=tol,
            args=(fail_parm, base_kw, mult),
        )  # ,**kw)
        if self._do_print:
            self.info(
    ...
    def determine_failure_front(
        ...
        if self._basis is None:
            ...
            self._basis = np.array(
                [
                    max([abs(v.get(p, 1e6)) for k, v in res.items()])
                    for p in self._prediction_parms
                ]
            )
        ...
        if pareto_front:
            for mult in mvec:
                for aux_frac in pareto_inx:
                    ...
                    for fail_parm, aux_parm in itertools.combinations(
                        self._prediction_parms, 2
                    ):
                        ...
                        self.solve_fail(
                            fail_parm, base_kw, guess=guesstimate, mult=mult
                        )
    ...
    def basis_expand(
        self,
        expand_values=[0.9, 0.75, 0.5, 0.1, 0.01],
        Nparm=4,
        est=True,
        normalize=True,
    ):
    ...
    def random_force_input(self, exp_mg=12, exp_min=6, min_ex=1, max_ex=3):
        porp = np.random.random(size=6) ** np.random.randint(
            numpy.random.randint(min_ex, max_ex),
            numpy.random.randint(exp_min, exp_mg),
            size=(6,),
        )
    ...
    def __hash__(self):
        ...
        vals = (
            shape.wkb_hex,
            str(hash(self.material)),
            str(self.goal_elements),
            str(self.min_mesh_angle),
        )
        h = hashlib.md5()
        for hv in vals:
            h.update(hv.encode())
